# /src/gui/streamlit/screens/notification_flow.py
...
from src.infrastructure.renderer.flow_diagram.diagram_nodes.notification_flow_nodes import (
    NOTIFICATION_FLOW_NODES,
    NOTIFICATION_FLOW_EDGES,
)
...
def render_patient_flow_screen() -> None:
    flow_nodes: dict[str, dict[str, str]] = (
        build_flow_diagram_node_dict(NOTIFICATION_FLOW_NODES))

    flow_edge: list[tuple[str, str, str | None]] = (
        build_flow_diagram_edge_tuple(NOTIFICATION_FLOW_EDGES))

    label_to_id_map: dict[str, str]  = (
        build_node_label_to_id_map(NOTIFICATION_FLOW_NODES)
    )

    st.title("Notifications Flow")
