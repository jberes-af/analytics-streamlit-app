# /src/gui/streamlit/screens/more_flow.py
...
from src.infrastructure.renderer.flow_diagram.diagram_nodes.more_flow_nodes import (
    MORE_FLOW_NODES,
    MORE_FLOW_EDGES,
)
...
def render_more_flow_screen() -> None:
    flow_nodes: dict[str, dict[str, str]] = (
        build_flow_diagram_node_dict(MORE_FLOW_NODES))

    flow_edge: list[tuple[str, str, str | None]] = (
        build_flow_diagram_edge_tuple(MORE_FLOW_EDGES))

    label_to_id_map: dict[str, str]  = (
        build_node_label_to_id_map(MORE_FLOW_NODES)
    )
