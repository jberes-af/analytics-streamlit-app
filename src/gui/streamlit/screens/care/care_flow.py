# /src/gui/streamlit/screens/care_flow.py
...
from src.infrastructure.renderer.flow_diagram.diagram_nodes.care_flow_nodes import (
    CARE_FLOW_NODES,
    CARE_FLOW_EDGES,
)
...
def render_care_flow_screen() -> None:
    flow_nodes: dict[str, dict[str, str]] = (
        build_flow_diagram_node_dict(CARE_FLOW_NODES))

    flow_edge: list[tuple[str, str, str | None]] = (
        build_flow_diagram_edge_tuple(CARE_FLOW_EDGES))

    label_to_id_map: dict[str, str]  = (
        build_node_label_to_id_map(CARE_FLOW_NODES)
    )
    ...
    """
    metadata = {
        "last_updated_at": CARE_FLOW_LAST_UPDATED_AT
    }

    import json

    bob = json.dumps(metadata)
    # st.caption(f"Last updated: {CARE_FLOW_LAST_UPDATED_AT}")
    """
