# /src/gui/streamlit/screens/adl_resident_flow.py
...
from src.infrastructure.renderer.flow_diagram.diagram_nodes.adl_resident_flow_nodes import (
    ADL_RESIDENT_FLOW_NODES,
    ADL_RESIDENT_FLOW_EDGES,
)
...
def render_adl_resident_flow_screen() -> None:
    flow_nodes: dict[str, dict[str, str]] = (
        build_flow_diagram_node_dict(ADL_RESIDENT_FLOW_NODES))

    flow_edge: list[tuple[str, str, str | None]] = (
        build_flow_diagram_edge_tuple(ADL_RESIDENT_FLOW_EDGES))

    label_to_id_map: dict[str, str]  = (
        build_node_label_to_id_map(ADL_RESIDENT_FLOW_NODES)
    )

    st.title("ADL Resident View")
