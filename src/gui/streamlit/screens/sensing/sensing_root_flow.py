# /src/gui/streamlit/screens/sensing_root_flow.py
...
from src.infrastructure.renderer.flow_diagram.diagram_nodes.sensing_root_flow_nodes import (
    SENSING_ROOT_FLOW_NODES,
    SENSING_ROOT_FLOW_EDGES,
)
...
def render_sensing_root_flow_screen() -> None:
    flow_nodes: dict[str, dict[str, str]] = (
        build_flow_diagram_node_dict(SENSING_ROOT_FLOW_NODES))

    flow_edge: list[tuple[str, str, str | None]] = (
        build_flow_diagram_edge_tuple(SENSING_ROOT_FLOW_EDGES))

    label_to_id_map: dict[str, str]  = (
        build_node_label_to_id_map(SENSING_ROOT_FLOW_NODES)
    )
