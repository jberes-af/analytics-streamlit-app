...
class AppConfigLoader:
    ...
    def _parse_paths(raw: Dict[str, Any], *, project_root: Path) -> PathsConfig:
        ...
        file_01_rel = p.get("placeholder_raw_csv_path")
        if not isinstance(file_01_rel, str) or not file_01_rel.strip():
            print(f"paths.placeholder_csv_path must be a non-empty string: {file_01_rel}")
            raise ValueError("paths.raw_csv_path must be a non-empty string")

        file_02_rel = p.get("placeholder_out_md")
        if not isinstance(file_02_rel, str) or not file_02_rel.strip():
            print(f"paths.placeholder_out_json must be a non-empty string: {file_01_rel}")
            raise ValueError("paths.placeholder_out_json must be a non-empty string")

        return PathsConfig(
            input_dir=(project_root / file_01_rel).resolve(),
